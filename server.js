...
function armTimer(code) {
  ...
  timers.set(code, setTimeout(() => {
    ...
    if (r.phase === "drawn") { r.log = `${r.players[r.turn].name} kept the card (time!).`; endTurnAdvance(r, false, 0); }
    else { doDraw(r, r.turn); if (r.phase === "drawn") { endTurnAdvance(r, false, 0); r.log = `${r.players[r.turn] ? "" : ""}Time! Card kept.`; } }
    bump(r); armTimer(code);
  }, TURN_MS));
}
...
io.on("connection", (socket) => {
  ...
  socket.on("draw", () => {
    const room = currentRoom();
    if (!room || room.status !== "playing" || room.phase !== "turn") return;
    const seat = room.players.findIndex((p) => p.id === socket.data.playerId);
    if (seat !== room.turn) return;
    doDraw(room, seat);
    bump(room);
  ...
  socket.on("keep", () => {
    const room = currentRoom();
    if (!room || room.status !== "playing" || room.phase !== "drawn") return;
    const seat = room.players.findIndex((p) => p.id === socket.data.playerId);
    if (seat !== room.turn) return;
